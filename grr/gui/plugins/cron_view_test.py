...
class TestCronView(test_lib.GRRSeleniumTest):
  ...
  def testDeprecatedHuntSchedulingWorksCorrectly(self):
    with test_lib.ConfigOverrider({
      ...
      # Select daily periodicity
      self.Type("css=grr-new-cron-job-deprecated-wizard-form "
                "label:contains('Periodicity') ~ * input", "1d")
      ...
      self.Type("css=grr-new-cron-job-deprecated-wizard-form "
                "grr-form-proto-repeated-field:has(label:contains('Paths')) "
                "input", "/tmp")
      self.Select("css=grr-new-cron-job-deprecated-wizard-form "
                  "grr-form-proto-single-field:has(label:contains('Pathtype')) "
                  "select", "TSK")
      ...
      self.Select("css=grr-new-cron-job-deprecated-wizard-form select",
                  "DummyOutputPlugin")
      self.Type(
          "css=grr-new-cron-job-deprecated-wizard-form "
          "grr-form-proto-single-field:has(label:contains('Filename Regex')) "
          "input", "some regex")
