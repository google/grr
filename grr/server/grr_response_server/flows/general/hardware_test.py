...
""".encode("utf-8")
    ...
    self.assertLen(results, 1)
    self.assertEqual(results[0].serial_number, "XY42EDVYNN")
    self.assertEqual(results[0].system_product_name, "MacBookPro18,3")
    self.assertEqual(results[0].bios_version, "10151.101.3")

    # TODO: Test more fields once we no longer depend on the legacy
    # parser.
