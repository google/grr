...
import {StatMultipleFilesArgs} from '../../../lib/api/api_interfaces';
...
import {StatMultipleFilesFormHarness} from './testing/stat_multiple_files_form_harness';

initTestEnvironment();

async function createComponent(fixedFlowArgs?: object, editable = true) {
  const fixture = TestBed.createComponent(StatMultipleFilesForm);
  if (fixedFlowArgs) {
...
describe('Stat Multiple Files Form Component', () => {
  beforeEach(waitForAsync(() => {
    TestBed.configureTestingModule({
      imports: [StatMultipleFilesForm, NoopAnimationsModule],
      providers: [
        {
  ...
  it('triggers onSubmit callback when submitting the form', async () => {
    ...
    fixture.componentRef.setInput(
      ...
      (flowName: string, flowArgs: object) => {
        expect(flowName).toBe('StatMultipleFiles');
  ...
  it('converts the form state to flow args', async () => {
    ...
    const expectedFlowArgs: StatMultipleFilesArgs = {
      pathExpressions: ['/some/path/with/trailing/spaces/tabs/and/linebreaks'],
      contentsLiteralMatch: {
        literal: btoa('test'),
      },
