...
describe('DateTimeInput Component', () => {
  ...
  it('reports an error for an invalid date/time input', () => {
    ...
    expect(fixture.componentInstance.formControl.errors).toEqual({
      invalidDateTime: `The input "invalid" can't be parsed as format ${DATE_TIME_FORMAT}`,
    });
  });
  ...
  it('reports a DateTime object for a valid date/time input', () => {
    ...
    const expectedValue = DateTime.fromISO('2021-01-02T03:04:05.000Z', {
      zone: 'UTC',
    });
  ...
  it('inserts selected date with calendar icon click in empty input', fakeAsync(async () => {
    jasmine.clock().mockDate(new Date('2020-07-01T13:00:00.000Z'));

    const fixture = createComponent();
    const loader = TestbedHarnessEnvironment.documentRootLoader(fixture);

    const calendarButton = fixture.debugElement.query(
      By.css('.calendar-toggle'),
    );
  ...
  it('inserts selected time with current date on clocks click in empty input', fakeAsync(async () => {
    jasmine.clock().mockDate(new Date('2020-07-01T13:00:00.000Z'));

    const fixture = createComponent();
    const loader = TestbedHarnessEnvironment.documentRootLoader(fixture);

    const clockMenuButton = fixture.debugElement.query(By.css('.clock-toggle'));
  ...
  it('updates existing input with date on calendar icon click', fakeAsync(async () => {
    jasmine.clock().mockDate(new Date('2020-07-01T13:00:00.000Z'));
  ...
  it('updates existing input with time on clocks icon click', fakeAsync(async () => {
    jasmine.clock().mockDate(new Date('2020-07-01T13:00:00.000Z'));
