...
export class ListProcessesForm extends FlowArgumentForm<
  ListProcessesArgs,
  Controls
> {
  ...
  readonly autocompleteStates$ = combineLatest([
    this.connectionStateAutocompleteControl.valueChanges.pipe(startWith('')),
  ...
  removeConnectionState(state: NetworkConnectionState) {
    const states = this.controls.connectionStates
      .value as NetworkConnectionState[];
    this.controls.connectionStates.setValue(
      states.filter((st) => st !== state),
    );
  }

  addConnectionState(state: NetworkConnectionState) {
    this.controls.connectionStates.setValue([
      ...this.controls.connectionStates.value,
      state,
    ]);
