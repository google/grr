...
describe('OsqueryQueryHelper component', () => {
  beforeEach(waitForAsync(() => {
    return TestBed.configureTestingModule({
      imports: [OsqueryQueryHelperModule, NoopAnimationsModule],
  ...
  it('should have queryToDisplay$ emit undefined when search is not a valid table name', fakeAsync(() => {
    const fixture = constructFixture();
    ...
    fixture.componentInstance.queryToReturn$
      .pipe(take(1))
      .subscribe((queryToReturn) => {
        expect(queryToReturn).toBeUndefined();
      });
  }));

  it('should have queryToDisplay$ emit proper string when search is a valid table name', fakeAsync(() => {
    const fixture = constructFixture();
  ...
  it("should have no elements in filteredCategories$ when search doesn't match anything", fakeAsync(() => {
    ...
    // This relies on the `shareReplay(1)` operator as part of the
    // `searchValues$` pipeline inside `osquery_query_helper.ts`.
    fixture.componentInstance.filteredCategories$
      .pipe(take(1))
