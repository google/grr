...
export class OsqueryResultsTableDOM {
  ...
  readonly columnsText?: ReadonlyArray<string> = this.columnElements
    ?.map(columnElement => columnElement.nativeElement.innerText);

  readonly cellDivs = this.rootElement?.queryAll(By.css('td'));
  readonly cellsText?: ReadonlyArray<string> = this.cellDivs
    ?.map(cellDiv => cellDiv.nativeElement.innerText);
