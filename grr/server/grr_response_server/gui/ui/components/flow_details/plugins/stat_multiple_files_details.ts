...
import {CollectMultipleFilesArgs, StatEntry} from '../../../lib/api/api_interfaces';
...
const ADAPTER: FlowResultMapFunction<readonly FlowFileResult[]|undefined> =
    (results) => results?.map(
        (result) => flowFileResultFromStatEntry(
...
export class StatMultipleFilesDetails extends Plugin {
  ...
  readonly flowResultsCount$ = this.flow$.pipe(map((flow) => {
    const resultsByType = flow?.resultCounts ?? [];

    const statEntryResultCount =
        resultsByType.find(count => count.type === PayloadType.STAT_ENTRY);

    return statEntryResultCount?.count ?? 0;
  }));

  readonly isResultsSectionExpandable$ =
      this.flowResultsCount$.pipe(map((resultsCount) => resultsCount > 0));
