...
const ADAPTER: FlowResultMapFunction<readonly FlowFileResult[] | undefined> = (
  ...
  results
    ?.map((item) => item.payload as FileFinderResult)
    .map((res) =>
      flowFileResultFromStatEntry(
...
export class FileFinderDetails extends Plugin {
  ...
  readonly args$: Observable<FileFinderArgs> = this.flow$.pipe(
    map((flow) => flow.args as FileFinderArgs),
  );

  readonly totalFileCount$: Observable<number | undefined> = this.flow$.pipe(
    map(
      (flow) =>
        flow.resultCounts?.find((rc) => rc.type === 'FileFinderResult')?.count,
    ),
  );
