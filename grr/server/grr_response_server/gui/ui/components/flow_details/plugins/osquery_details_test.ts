...
import {
  newOsqueryTable,
  OsqueryResultsTableDOM,
} from '../../../components/flow_details/helpers/osquery_test_util';
...
describe('osquery-details component', () => {
  ...
  it('should display progress table, and a button with count to request more if results are truncated', () => {
    ...
    expect(parsedElements.showAdditionalButtonText).toBe(
      'View all rows (1 more)',
    );
  });
  ...
  it('should display the results table instead of the progress table when both present', () => {
    ...
    expect(parsedElements.displayedTable?.cellsText).toEqual(
      resultCells.flat(),
    );
  });
