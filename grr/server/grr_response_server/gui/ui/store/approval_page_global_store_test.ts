...
describe('ApprovalPageGlobalStore', () => {
  ...
  it('calls the subscribeToClientApproval API on approval$ subscription', fakeAsync(() => {
    approvalPageGlobalStore.selectApproval({
      clientId: 'C.1234',
      requestor: 'testuser',
      approvalId: '123',
    ...
    expect(httpApiService.subscribeToClientApproval).toHaveBeenCalledWith({
      clientId: 'C.1234',
      requestor: 'testuser',
      approvalId: '123',
    });
  ...
  it('calls the API on grantApproval()', async () => {
    ...
    expect(
      await firstValueFrom(approvalPageGlobalStore.grantRequestStatus$),
