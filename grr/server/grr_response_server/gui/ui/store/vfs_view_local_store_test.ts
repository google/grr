...
const apiFile = (
  path: string,
  pathtype: PathSpecPathType = PathSpecPathType.OS,
): ApiFile => ({
...
describe('VfsViewLocalStore', () => {
  ...
  it('navigateToPath lists files from API', async () => {
    ...
    httpApiService.browseFilesystem = jasmine
      .createSpy('browseFilesystem')
      .and.callFake(browseFilesystem);
    ...
    expect(httpApiService.browseFilesystem).toHaveBeenCalledOnceWith(
      'C.1234',
      '/',
    ...
    expect(tree).toEqual(
      jasmine.objectContaining({
        path: '/',
        children: [jasmine.objectContaining({path: '/foo'})],
      }),
    );
  });
  ...
  it('directory loading is indicated in loading boolean', async () => {
    ...
    expect(await firstValueFrom(vfsViewLocalStore.directoryTree$)).toEqual(
      jasmine.objectContaining({
        path: '/',
        loading: true,
      }),
  ...
  it('listCurrentDirectory calls refreshVfsFolder', async () => {
    ...
    expect(httpApiService.refreshVfsFolder).toHaveBeenCalledOnceWith(
      'C.1234',
      PathSpecPathType.OS,
      '/',
      undefined,
  ...
  it('deep listCurrentDirectory refreshes expanded sub-directories', async () => {
    expect(httpApiService.refreshVfsFolder).not.toHaveBeenCalled();
  ...
  it('listCurrentDirectory updates isListingCurrentDirectory$', async () => {
    expect(
      await firstValueFrom(vfsViewLocalStore.isListingCurrentDirectory$),
    ).toBeFalse();
        ...
        if (path === '/foo') {
          return of({
            items: [
              ...
              {
                path: '/foo',
                children: [apiDir('/foo/bar'), apiDir('/foo/baz')],
              },
            ],
          });
        } else {
  ...
  it('currentDirectory$ emits parent directory if file is selected', async () => {
    httpApiService.browseFilesystem = jasmine
      .createSpy('browseFilesystem')
      .and.callFake((clientId, path) => {
        if (path === '/foo/barfile') {
          return of({
            items: [
              ...
              {
                path: '/foo',
                children: [apiFile('/foo/barfile'), apiDir('/foo/baz')],
              },
            ],
          });
        } else {
