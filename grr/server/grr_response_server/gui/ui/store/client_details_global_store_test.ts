...
describe('ClientDetailsGlobalStore', () => {
  ...
  beforeEach(() => {
    ...
    httpApiService = {
      fetchClientVersions: jasmine
        .createSpy('fetchClientVersions')
  ...
  it('getClientVersions() correctly translates snapshots into client changes', () => {
    ...
    const expectedClientChanges = [
      ...
      },
      {
        client: snapshots[2],
        changes: [
          '2 User full name entries added',
          'User home directory added',
        ],
      },
      {
        client: snapshots[3],
    ...
    expect(
      ...
      expectedClientChanges.map((expectedChange) => [
