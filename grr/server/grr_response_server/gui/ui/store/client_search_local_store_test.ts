...
describe('ClientSearchLocalStore', () => {
  ...
  it('fetches new search results from API when "searchClients" is called with a non-empty query, and sorts them by when they were last seen', async () => {
  ...
  it('does not fetch new search results from the API when "searchClients" is called with an empty query', async () => {
    const clientSearchLocalStore = TestBed.inject(ClientSearchLocalStore);
    const httpApiService = injectHttpApiServiceMock();
