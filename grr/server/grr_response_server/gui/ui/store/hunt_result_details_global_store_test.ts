...
import {ApiFlow, ApiFlowState, ApiHuntError, ApiHuntResult} from '../lib/api/api_interfaces';
...
import {HuntResultDetailsGlobalStore, RESULT_BATCH_COUNT, stringifyAndBeautify} from './hunt_result_details_global_store';
...
describe('Hunt Result Details Global Store', () => {
  ...
  describe('Loading a Hunt Result', () => {
    it('Tries to fetch the hunt result after receiving a Hunt Result Key',
       async () => {
         ...
         huntResultDetailsGlobalStore.selectHuntResultId(
             mockHuntResultKey, PayloadType.USER);
         ...
         const huntId =
             await firstValueFrom(huntResultDetailsGlobalStore.huntId$);
         expect(huntId).toEqual(mockHuntId);

         const clientId =
             await firstValueFrom(huntResultDetailsGlobalStore.clientId$);
         expect(clientId).toEqual(mockHuntResult.clientId);

         const timestamp =
             await firstValueFrom(huntResultDetailsGlobalStore.timestamp$);
    ...
    it('Successful search for a hunt result details', async () => {
      ...
      expect(displayValue)
          .toEqual(stringifyAndBeautify(mockHuntResultDetails.payload));
    ...
    it('Does not try to load a hunt result if it is already present',
       async () => {
         const mockHuntResult = {
           clientId: 'C.1234',
           timestamp: '1',
         ...
         huntResultDetailsGlobalStore.selectHuntResultOrError(
             mockHuntResultDetails, mockHuntId);
    ...
    it('Reloads the hunt result details after receiving a new result key Id',
       async () => {
         const mockHuntResult = {
           clientId: 'C.1234',
           timestamp: '1',
         ...
         const anotherMockHuntResultKey =
             getHuntResultKey(anotherMockHuntResult, mockHuntId);

         huntResultDetailsGlobalStore.selectHuntResultId(
             anotherMockHuntResultKey, PayloadType.FILE_FINDER_RESULT);
         ...
         const anotherTimestamp =
             await firstValueFrom(huntResultDetailsGlobalStore.timestamp$);
         expect(anotherTimestamp)
             .toEqual(createDate(anotherMockHuntResult.timestamp));
       });
  });

  describe('Loading a Hunt Error', () => {
    it('Tries to fetch the hunt error after receiving a Hunt Result Key',
       fakeAsync(() => {
         const mockHuntResult = {
           clientId: 'C.1234',
           timestamp: '1',
         ...
         huntResultDetailsGlobalStore.selectHuntResultId(
             mockHuntResultKey, PayloadType.API_HUNT_ERROR);
    ...
    it('Successful search for a hunt error details', async () => {
      ...
      const displayValue = await firstValueFrom(
          huntResultDetailsGlobalStore.resultOrErrorDisplay$);
      expect(displayValue).toEqual(stringifyAndBeautify({
    ...
    it('Does not try to load a hunt error if it is already present',
       async () => {
         const mockHuntResult = {
           clientId: 'C.1234',
           timestamp: '1',
         ...
         huntResultDetailsGlobalStore.selectHuntResultOrError(
             mockHuntErrorDetails, mockHuntId);
    ...
    it('Reloads the hunt error details after receiving a new result key Id',
       async () => {
         const mockHuntResult = {
           clientId: 'C.1234',
           timestamp: '1',
         ...
         huntResultDetailsGlobalStore.selectHuntResultId(
             anotherMockHuntResultKey, PayloadType.API_HUNT_ERROR);
         ...
         const displayValue2 = await firstValueFrom(
             huntResultDetailsGlobalStore.resultOrErrorDisplay$);
         expect(displayValue2).toEqual(stringifyAndBeautify({
  ...
  describe('Flow Details', () => {
    it('Loads the flow of a Hunt Result, with its descriptor', async () => {
      ...
      expect(httpApiService.fetchFlow)
          .toHaveBeenCalledWith(mockHuntResult.clientId, mockHuntId);
    ...
    it('Re-fetches the flow after receiving a new result key Id', async () => {
      ...
      expect(httpApiService.fetchFlow)
          .toHaveBeenCalledWith(anotherMockHuntResult.clientId, mockHuntId);
