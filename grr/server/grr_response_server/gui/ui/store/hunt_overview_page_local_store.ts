...
@Injectable()
export class HuntOverviewPageLocalStore extends ApiCollectionStore<
  Hunt,
  ApiListHuntsArgs
> {
  ...
  protected loadResults(
    ...
    return this.httpApiService
      ...
      .pipe(map((response) => (response.items ?? []).map(translateHunt)));
  }

  protected readonly compareItems = compareDateNewestFirst<Hunt>(
    (hunt) => hunt.created,
  );
