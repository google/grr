import {tick as angularTick, fakeAsync} from '@angular/core/testing';
...
describe('queuedExhaustMap', () => {
  ...
  it("shouldn't discard anything if there are no concurrent elements (queue size 1)", () => {
    ...
    const sentValues = Array.from<number>({length: repeatTimes}).map(
      (value, index) => index,
    );
  ...
  it('should discard everything but the last concurrent element (queue size 1)', fakeAsync(() => {
    ...
    const manyValuesWithDelay$ = of(
      ...
      queuedExhaustMap((value) => {
        return of(value).pipe(delay(blockTimeMs));
      }, 1),
    );
  ...
  it('should discard all but the last emissions of the same element concurrent with something else (queue size 1)', fakeAsync(() => {
    const blockTimeMs = 1000;
    const infTime = blockTimeMs * 100;
    const sameElement = 'this is repeated multiple times';

    const sameValueWithDelay$ = of(
      sameElement,
      sameElement,
      sameElement,
      sameElement,
...
function emittedValuesWatcher<T>(
