...
import {
  ...
  compareDateNewestFirst,
  compareDateOldestFirst,
  mergeMaps,
  toMap,
} from './type_utils';
...
describe('mergeMaps', () => {
  it('merges two maps', () => {
    expect(mergeMaps(mapOf({a: 1, b: 2}), mapOf({c: 3}))).toEqual(
      mapOf({a: 1, b: 2, c: 3}),
    );
  });
  ...
  it('merges many maps', () => {
    expect(
      mergeMaps(mapOf({a: 1, b: 2}), mapOf({c: 3}), mapOf({d: 4})),
...
describe('toMap', () => {
  it('applies keymapper', () => {
    expect(toMap([1, 2, 3], (i) => `key${i}`)).toEqual(
      mapOf({'key1': 1, 'key2': 2, 'key3': 3}),
    );
  });
