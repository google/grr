...
export type RequestStatus<Success = undefined, Error = HttpErrorResponse> =
  | SentRequestStatus
  | SuccessRequestStatus<Success>
  | ErrorRequestStatus<Error>;
  ...
  return merge(
    ...
    catchError((error) => {
      if (error instanceof HttpErrorResponse) {
        return of<ErrorRequestStatus>({status: RequestStatusType.ERROR, error});
      } else {
...
export function extractErrorMessage<S>(
  ...
  if (requestStatus.status === RequestStatusType.ERROR) {
    return {
      ...
      error: (
        requestStatus.error.error?.message ?? requestStatus.error.message
      ).toString(),
    };
  } else {
