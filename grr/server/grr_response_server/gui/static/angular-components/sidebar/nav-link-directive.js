...
const NavLinkController = class {
   ...
   * @ngInject
   */
  constructor($scope, $element, grrRoutingService) {
    ...
    this.scope_.$watchGroup(
        ['state', 'params'], this.onStateOrParamsChange_.bind(this));

    this.grrRoutingService_.onStateChange(
        this.scope_, this.applyActiveState_.bind(this));
  }

  /**
   * @private
   */
  onStateOrParamsChange_() {
    this.href = '#';
    if (angular.isDefined(this.scope_['state'])) {
      this.href = this.grrRoutingService_.href(
          this.scope_['state'], this.scope_['params']);
    }
  }
