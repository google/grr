 ...
 * @unrestricted
 */
const BreadcrumbsController = class {
  /**
  ...
  constructor($rootScope, $scope, grrApiService) {
    ...
    this.scope_.$watchGroup(
        ['path', 'stripEndingSlash'], this.onDirectiveArgsChange_.bind(this));
  }
  ...
  onDirectiveArgsChange_() {
    ...
    // grr-breadcrumbs displays all the containing folders. So the last
    // component is supposed to be neglected as it's the filename of the actual
    // file, when we only care about the containing directories. Therefore
    // having 1 or 0 components means invalid input and we display nothing.
...
exports.BreadcrumbsDirective = function() {
  return {
    ...
    templateUrl:
        '/static/angular-components/client/virtual-file-system/breadcrumbs.html',
