 ...
 * @unrestricted
 */
const FileTreeController = class {
  /**
  ...
  constructor($rootScope, $scope, $element, grrApiService, grrRoutingService) {
    ...
    this.scope_.$on(
        REFRESH_FOLDER_EVENT, this.onRefreshFolderEvent_.bind(this));

    this.scope_.$watch(
        'controller.fileContext.clientId', this.onClientIdChange_.bind(this));
    this.scope_.$watch(
    ...
    this.treeElement_.on('changed.jstree', function(e, data) {
      ...
      if (getFolderFromPath(this.fileContext['selectedFilePath']) ===
        ...
        this.rootScope_.$broadcast(
            REFRESH_FOLDER_EVENT, ensurePathIsFolder(folderPath));
      } else {
   ...
   * @return {!angular.$q.Promise} A promise returning the child files when
   *     resolved.
   * @private
   */
  ...
  /**
   * Selects a folder defined by the given path. If the path is not available,
   * it selects the closest parent folder.
   *
   ...
   * @private
   */
  expandToFilePath_(filePathId, opt_suppressEvent) {
    if (!filePathId) {
      return;
    }
    ...
    var cb = function(i, prev_node) {
      ...
      if (node.length) {
        if (parts[i + 1]) {
          ...
          element.jstree('open_node', node, function() {
            cb(i + 1, node);
          }, 'no_hash');
        } else {
...
exports.FileTreeDirective = function() {
  return {
    ...
    templateUrl:
        '/static/angular-components/client/virtual-file-system/file-tree.html',
