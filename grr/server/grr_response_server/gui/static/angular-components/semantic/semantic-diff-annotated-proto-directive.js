 ...
 * @unrestricted
 */
const SemanticDiffAnnotatedProtoController = class {
  /**
  ...
  onValueChange_(newValue, oldValue) {
    ...
    if (angular.isObject(this.scope_['value'])) {
      this.grrReflectionService_
          .getRDFValueDescriptor(this.scope_['value']['type'])
          .then(
              // TODO(user): Reflection failure scenario should be
              // handled globally by reflection service.
              function success(descriptor) {
                this.items = buildItems(
                    this.scope_['value'], descriptor,
                    this.scope_['visibleFields'], this.scope_['hiddenFields']);
