...
class CronManager(object):
  ...
  def RunJob(self, job: rdf_cronjobs.CronJob) -> None:
    ...
    if job.args.action_type == job.args.ActionType.SYSTEM_CRON_ACTION:
      ...
      raise ValueError("Hunt cron jobs are deprecated: %s" % job.cron_job_id)
    else:
      raise ValueError(
...
def ScheduleSystemCronJobs(names: Optional[Sequence[str]] = None) -> None:
  ...
  for name in names:
    ...
    system = flows_pb2.CronJobAction.ActionType.SYSTEM_CRON_ACTION
    args = flows_pb2.CronJobAction(
        action_type=system,
        system_cron_action=flows_pb2.SystemCronAction(job_class_name=name),
    )
    job = flows_pb2.CronJob(
        cron_job_id=name,
        args=args,
        enabled=enabled,
        frequency=cls.frequency.SerializeToWireFormat(),
        lifetime=cls.lifetime.SerializeToWireFormat(),
...
class RunHunt(CronJobBase):
  ...
  def Run(self):
    raise NotImplementedError("Hunt cron jobs are deprecated.")
