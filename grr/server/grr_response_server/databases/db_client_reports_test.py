...
class DatabaseTestClientReportsMixin(object):
  ...
  def testReadAllClientGraphSeries_InTimeRange(self):
    ...
    for i, graph_series in enumerate(graph_series_list):
      with test_lib.FakeTime(date + rdfvalue.Duration.From(i, rdfvalue.DAYS)):
        self.db.WriteClientGraphSeries(graph_series, _TEST_LABEL)
    ...
    expected_data = {
        date + rdfvalue.Duration.From(6, rdfvalue.DAYS): graph_series_list[6],
        date + rdfvalue.Duration.From(7, rdfvalue.DAYS): graph_series_list[7],
        date + rdfvalue.Duration.From(8, rdfvalue.DAYS): graph_series_list[8],
        date + rdfvalue.Duration.From(9, rdfvalue.DAYS): graph_series_list[9],
    }
    self.assertDictEqual(fetched_data, expected_data)
