...
def make_ui_files():
  ...
  subprocess.check_call("npm ci", shell=True, cwd="grr_response_server/gui/static")
...
class Develop(develop):
  ...
  def run(self):
    ...
    if not self.no_make_ui_files and not os.environ.get(GRR_NO_MAKE_UI_FILES_VAR):
      make_ui_files()
...
data_files = list(
  itertools.chain(
    ...
    find_data_files("grr_response_server/gui/static", ignore_dirs=IGNORE_GUI_DIRS),
    find_data_files(
...
setup_args = dict(
  ...
  entry_points={
    "console_scripts": [
      ...
      ("grr_config_updater = " "grr_response_server.distro_entry:ConfigUpdater"),
  ...
  install_requires=[
    ...
    "grr-response-client-builder==%s" % VERSION.get("Version", "packagedepends"),
