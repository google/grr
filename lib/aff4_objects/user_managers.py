...
class CoreAccessControlManager(BaseAccessControlManager):
  """Basic ACL manager that uses the config file for user management."""

  SYSTEM_ACCOUNTS = ['GRREnroller', 'GRRCron', 'GRRWorker', 'GRRSystem', 'GRRFrontEnd']
  CLIENT_URN_PATTERN = "aff4:/C." + "[0-9a-fA-F]" * 16
  ...
  def __init__(self):
    ...
    super(CoreAccessControlManager, self).__init__()
  ...
  def CheckFlowAccess(self, token, flow_name, client_id=None):
    ...
    if not flow_cls.ACL_ENFORCED:
      logging.debug("Access granted by ACL_ENFORCED bypass for %s.", flow_name)
      return True
    ...
    return self.CheckACL(token=token, target=client_urn)

  @stats.Timed("acl_check_time")
  def CheckHuntAccess(self, token, hunt_urn):
    return self.CheckACL(token=token, target=hunt_urn)

  @stats.Timed("acl_check_time")
  def CheckCronJobAccess(self, token, cron_job_urn):
    return self.CheckACL(token=token, target=cron_job_urn)
  ...
  def CheckDataStoreAccess(self, token, subjects, requested_access="r"):
    ...
    logging.debug("Checking %s access to %s on %s. Supervisor: %s", requested_access, token.username, subjects, token.supervisor)
    
    # The supervisor may bypass all ACL checks.
    if token.supervisor:
      logging.debug("%s access granted to %s on %s. Supervisor: %s", requested_access, token.username, subjects, token.supervisor)   
      return True
    ...
    for subject in subjects:
      for access in requested_access:
        try:
          ...
          logging.warn("%s access rejected for %s on %s: %s", requested_access,
                       token.username, subject, e)
          e.requested_access = requested_access
          raise
  ...
  def CheckACL(self, target, token):
    self.ValidateToken(token=token, target=target)
    logging.debug("Access granted to %s on %s. Supervisor: %s", token.username, target, token.supervisor)    
    return True


class AdminUIAccessControlManager(CoreAccessControlManager):
  ...
  def CheckACL(self, target, token):
    self.ValidateToken(token=token, target=target)
    logging.info("Checking access to %s on %s. Supervisor: %s", token.username, target, token.supervisor)
    
    # The supervisor may bypass all ACL checks.
    if token.supervisor:
      logging.info("Access granted to %s on %s. Supervisor: %s", token.username, target, token.supervisor)    
      return True

    #target may be None for flows not specifying a client.  Only SYSTEM_ACCOUNTS can run these flows.
    if not target:
      if token.username not in self.SYSTEM_ACCOUNTS:
        raise access_control.UnauthorizedAccess(
            "Access denied for flow without client_urn for %s", token.username)
