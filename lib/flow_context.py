...
class FlowContext(object):
  ...
  def __init__(self, client_id=None, flow_name=None,
               ...
               event_id=None, state=None, args=None, _store=None,
               token=None):
    ...
    self.token = token or data_store.ACLToken()
  ...
  def CallState(self, messages=None, next_state=""):
    ...
    with FlowManager(self.session_id, token=self.token,
      ...
      request_state = jobs_pb2.RequestState(id=self.next_outbound_id,
  ...
  def ProcessCompletedRequests(self, unused_thread_pool, unused_messages=None):
    ...
    Args:
      unused_thread_pool: For regular flows, the messages have to be processed
                          in order. Thus, the thread_pool argument is not used.
      unused_messages: These are the messages that came in from the
                       client this time. For regular flows these are unused
                       because the real messages are queues in the flow_pb.
    """
  ...
  def _ProcessSingleRequest(self, request, responses):
    ...
    try:
      ...
      getattr(self.parent_flow, self.current_state)(request=request,
                                                    responses=responses)
  ...
  def CallFlow(self, flow_factory, flow_name, next_state=None,
    ...
    child = flow_factory.StartFlow(self.client_id, flow_name,
  ...
  def Error(self, backtrace=None):
    ...
    if self.flow_pb.state == jobs_pb2.FlowPB.RUNNING:
      ...
      if backtrace:
        logging.error("Error in flow %s. Trace: %s", self.session_id, backtrace)
...
class FlowManager(object):
  ...
  FLOW_RESPONSE_PREFIX = "flow:response:%04X:"
  ...
  def FetchRequestsAndResponses(self):
    ...
    for predicate, serialized, _ in sorted(self.data_store.ResolveRegex(
        subject, self.FLOW_MESSAGE_REGEX, token=self.token)):
    ...
    for request_id in sorted(state_map):
      try:
        ...
        yield (metadata["REQUEST_STATE"], metadata["RESPONSES"])
      except KeyError:
        pass
