#ifndef UTIL_SIG_SCOPED_PTR_SSL_H__
#define UTIL_SIG_SCOPED_PTR_SSL_H__
...
class scoped_ptr_openssl_void {
 public:
  ...
  OpenSSLType *operator->() const {
    return ptr_;
  }
  OpenSSLType *get() const {
    return ptr_;
  }
  OpenSSLType *release() {
...
class ScopedOpenSSLErrorStackClearer {
 public:
  ~ScopedOpenSSLErrorStackClearer() {
    ERR_clear_error();
  }
};

#endif  // UTIL_SIG_SCOPED_PTR_SSL_H__
